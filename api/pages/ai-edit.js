  ...
  try{
    const r = await fetch('https://api.llama.com/compat/v1/chat/completions',{
      ...
      body: JSON.stringify({
        ...
        messages:[
          {role:'system',content:'너는 GEO 카피라이터. JSON만 리턴: {"h2":"...","h3":"...","body":"...","bullets":["..."],"layout":"left|right|grid|full"}'},
    ...
    return res.status(200).json(JSON.parse(content.replace(/```json|```/g,'').trim()));
